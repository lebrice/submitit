...
@pytest.fixture
def mocked_slurm() -> tp.Generator[test_core.MockedSubprocess, None, None]:
    mock = test_core.MockedSubprocess(known_cmds=["srun"])
    try:
        with mock.context():
            yield mock
    finally:
        # Clear the state of the shared watchers
        for watcher in slurm_remote.RemoteSlurmJob.watchers.values():
            watcher.clear()


def test_mocked_missing_state(
    mocked_slurm: test_core.MockedSubprocess, cluster: str
) -> None:
    mocked_slurm.set_job_state("12", "       ")
    job: slurm_remote.RemoteSlurmJob[None] = slurm_remote.RemoteSlurmJob(
        cluster, "temp_file", "12"
    )
    assert job.state == "UNKNOWN"
    job._interrupt(timeout=False)  # check_call is bypassed by MockedSubprocess


def test_job_environment(mocked_slurm: test_core.MockedSubprocess) -> None:
    mocked_slurm.set_job_state("12", "RUNNING")
    with mocked_slurm.job_context("12"):
        assert job_environment.JobEnvironment().cluster == "slurm"


@pytest.fixture
def temp_remote_path(cluster: str):
    return "bob"


def test_slurm_job_mocked(
